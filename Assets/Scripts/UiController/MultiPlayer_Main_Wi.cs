...
public class MultiPlayer_Main_Wi : WindowUi
{
    ...
    public void Join()
    {
        ...
        MultiPlayer_Main.m_sinThis.CreteOrJoin(m_inNameL.text, m_inNameP.text,() =>
        {
            ...
            var n = PhotonNetwork.Instantiate($"Cars/{m_gmPrebsCar[PlayerInfo.m_inId].name}",new Vector3(x,m_trStarPos.position.y,z),Quaternion.identity);
            if (n.GetPhotonView().IsMine)
            {
                ...
                {
                    Ui_Controller.m_sinThis.enabled = true;
                }
                n.GetComponent<Main_Car_Controller>().m_acCallBack += () =>
                {
